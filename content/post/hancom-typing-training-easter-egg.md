...
## 시작하기 전에
이 글은 한컴타자연습 2005를 기준으로 작성되었습니다. 2002, 2004 버전도 비슷한 구조를 갖고 있을 것입니다.
...
## 리버싱을 마치며
이 글에는 이스터에그 분석 내용만 적었는데, 세이브파일의 형식이나 프로그램의 전반적인 구성도 분석했습니다.
제가 WinAPI 개발을 해본 적이 없어서 평가할 자격은 없는데, 코드가 전반적으로 꽤 특이하게 작성되어있는 것 같습니다.
예를 들어, 버튼을 이미지 리소스로 그린 다음, 마우스 클릭 이벤트가 발생했을 때 마우스의 좌표로 어떤 버튼이 클릭됐는지 판별하는데,
이게 맞나 싶습니다. 어쨌든 잘 작동하니까 그만입니다.

사실 제가 글 쓰는 것을 잘 못합니다. 머릿속에 있는 생각을 글로 옮기면 두서가 없고 일기 힘든 문장이 나옵니다.
한 번 쓴 단어나 표현을 계속 반복해서 쓰는 습관도 있습니다. 이 글이 블로그에 올라가는 첫 글일텐데, 사실 쓰다가 도저히
안되겠다 싶어서 갖다 버린 글도 두세게 있고, 이 글도 처음 작성할 때 일주일 정도 붙들었습니다. 2025년 4월 14일, 오랜만에
방치해둔 제 블로그를 다시 보다가 잘 읽히지 않아 수정해야 할 부분이 있어 2년만에 다시 다듬었습니다.
